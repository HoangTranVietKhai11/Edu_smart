...
const navItems = [
  ...
  { to: '/teacher/classes', icon: Users, label: 'Lớp học' },
  { to: '/teacher/documents', icon: BookOpen, label: 'Tài liệu' },
  { to: '/teacher/exams', icon: FileText, label: 'Kiểm tra' },
  { to: '/teacher/announcements', icon: Bell, label: 'Thông báo' },
  { to: '/teacher/ai-chat', icon: Bot, label: 'AI Assistant' },
...
const mobileNavItems = [
  ...
  { to: '/teacher/documents', icon: BookOpen, label: 'Tài liệu' },
  { to: '/teacher/exams', icon: FileText, label: 'Kiểm tra' },
  { to: '/teacher/ai-chat', icon: Bot, label: 'AI Chat' },
];
    ...
    >
      <AppShell.Header>
        <Group h="100%" px="md" justify="space-between">
          <Group>
            {/* Burger is hidden completely now because we use the bottom bar menu on mobile */}
          ...
          <Group>
            <Link to="/teacher/dashboard" className="text-sm text-slate-500 hover:text-blue-600 flex items-center gap-1 transition-colors hidden sm:flex">
            ...
            <Menu shadow="md" width={200} position="bottom-end">
              ...
              <Menu.Dropdown>
                <Menu.Label>Tài khoản Giáo viên</Menu.Label>
                <Menu.Item 
          ...
          {navItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                `}
              >
                <item.icon size={20} strokeWidth={2.5} className={isActive ? 'text-blue-600' : 'text-slate-400'} />
                {item.label}
              </NavLink>
            );
          })}
      ...
      <div className="sm:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 flex items-center justify-around z-40 pb-safe">
        {mobileNavItems.map((item) => {
          ...
          return (
            <Link 
              key={item.to} 
              to={item.to} 
              className={`flex flex-col items-center justify-center w-16 py-2 transition-colors ${isActive ? 'text-blue-600' : 'text-slate-400'}`}
            >
        ...
        <UnstyledButton 
          onClick={toggle} 
          className={`flex flex-col items-center justify-center w-16 py-2 transition-colors ${opened ? 'text-blue-600' : 'text-slate-400'}`}
        >
